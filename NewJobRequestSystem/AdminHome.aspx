<%@ Page Title="Home" Language="C#" MasterPageFile="~/JOMain.Master" AutoEventWireup="true" CodeBehind="AdminHome.aspx.cs" Inherits="NewJobRequestSystem.AdminHome" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script>
        //$(document).ready(function(){
        //	$('.ddlSummaryYear').change(function(){
        //        var myselectedtxt = $(this).find("option:selected").val();
        //  		$('.myselectedtxtdiv').html("The selected text is:<strong> "+myselectedtxt+"</strong>"); 
        //  	});
        //});

        $(document).ready(function () {
            $find('ddlSummaryYear').add_propertyChanged(function (sender, event) {
                console.log(event);
                console.log(sender);
                if (event.get_propertyName() == 'selectedIndex') {
                    var newValue = sender.get_textBoxControl().value;
                    $('#test').val("You entered: " + newValue);
                }
            });
        });
    </script>
