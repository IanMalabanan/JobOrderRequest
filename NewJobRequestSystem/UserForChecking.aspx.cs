...
{
    public partial class UserForChecking : System.Web.UI.Page
    {
        ...
        {
            var signerQuery = dbContext.tblEmailLogs.Where(x => x.JR_Code == jrcode && x.UserRole.Trim() == "Checker").ToList();
        ...
        private void GetRequestDetails()
        {
            ...
            {
                tbdateRequest.Text = Convert.ToDateTime(row[0]).ToString("dd") + "-" 
                    + Convert.ToDateTime(row[0]).ToString("MMM") + "-" 
                    + Convert.ToDateTime(row[0]).ToString("yyyy");
                ...
                lblDateSigned_Checker.InnerText = string.IsNullOrEmpty(row[30].ToString()) ? string.Empty : Convert.ToDateTime(row[30]).ToString("yyyy-MMM-dd");

                lblDateSigned_Assesstor.InnerText = string.IsNullOrEmpty(row[31].ToString()) ? string.Empty : Convert.ToDateTime(row[31]).ToString("yyyy-MMM-dd");

                lblDateSigned_Noter.InnerText = string.IsNullOrEmpty(row[32].ToString()) ? string.Empty : Convert.ToDateTime(row[32]).ToString("yyyy-MMM-dd");

                lblDateSigned_Approver.InnerText = string.IsNullOrEmpty(row[33].ToString()) ? string.Empty : Convert.ToDateTime(row[33]).ToString("yyyy-MMM-dd");

                lblDateSigned_Receiver.InnerText = string.IsNullOrEmpty(row[34].ToString()) ? string.Empty : Convert.ToDateTime(row[34]).ToString("yyyy-MMM-dd");
            }
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            btnChangeNoter.Attributes["onclick"] = String.Format("return ShowChangeApproverForm('{0}','{1}');"
                , Request.QueryString["RCode"].ToString().Trim(), "Checker");
        }
