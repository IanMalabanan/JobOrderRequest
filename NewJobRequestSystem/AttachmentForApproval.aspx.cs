...
{
    public partial class AttachmentForApproval : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            btnShowReject.Attributes["onclick"] = String.Format("return ShowRejectForm('{0}','{1}','{2}');"
                , Request.QueryString["EmpID"].ToString().Trim(), Request.QueryString["RCode"].ToString().Trim(), "Approver_Attachment");
            ...
            var verifyRequest = dbContext.tblAttachments.Where(x => x.jr_code == jrcode && x.typeofattachment == "Admin" && x.IsPrepared == true && x.IsChecked == true && x.IsApproved == true).Count();
            ...
            var verifyRequest3 = dbContext.tblAttachments.Where(x => x.jr_code == jrcode && x.typeofattachment == "Admin" && x.IsPrepared == true && x.IsChecked == true && x.IsApproved == false).Count();
            ...
            //}
            //if(dbContext.tblAttachments.Where(x => x.jr_code == jrcode //&& x.typeofattachment == "Admin" 
            //&& x.IsPrepared == true && x.IsChecked == true && x.IsApproved == false).Count() > 0)
            //{
            ...
            if(app != null)
            {
                if(app.AttachementStatus == "NOTED")
                {
                    divSuccess.Visible = false;
        ...
        protected void DownloadFile(object sender, EventArgs e)
        {
            ...
            if (accept != null)
            {
                accept.IsApproverReviewed = true;
        ...
        private string GenerateJONumber()
        {
            string jo = string.Empty;

            SqlCommand cmd = new SqlCommand("GenerateJRFNo");

            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.AddWithValue("@inchargeid", incharge);

            foreach (DataRow dr in SqlHelper.ExecuteDataReader(ClsConfig.JobRequestConnectionString, cmd).Rows)
            {
                jo = dr[0].ToString().Trim();
            }
        ...
        protected void Approve(object sender, EventArgs e)
        {
            ...
            var count = dbContext.tblAttachments.Where(x => x.jr_code == jrcode
                        && x.typeofattachment.Trim() == "Admin" && x.IsCheckerReviewed == true && x.IsApproverReviewed == false).Count();
            ...
            else
            {
                ...
                if (accept != null)
                {
                    accept.ApprovedBy = approverfullname_fnamefirst;

                    accept.IsApproved = true;
                ...
                foreach (DataRow dr in ConvertToDataTable(asstr).Rows)
                {
                    ...
                    body += "<br /><br />" + "Attachment/s for a new job order request has/have been signed and approved by the manager";
                    ...
                    using (MailMessage mm = new MailMessage())
                    {
                        string sub = "Online Job Order Request: Approved Attachments";
        ...
        private void SendToRequestor()
        {
            ...
            body += "<br /><br />" + "Please be informed that your job order request has been approved.";
